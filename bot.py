...
async def global_text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Routes text: manga state first, then anime search."""
    handled = await handle_manga_text(update, ctx)
    if not handled:
        await search.handle_text(update, ctx)
...
def main():
    ...
    # ── Manga / Manhwa Commands ────────────────────────────────
    app.add_handler(CommandHandler("manga",   cmd_manga))
    app.add_handler(CommandHandler("manhwa",  cmd_manga))
    app.add_handler(CommandHandler("webtoon", cmd_manga))
    app.add_handler(CommandHandler("manhua",  cmd_manga))

    # ── Callbacks ──────────────────────────────────────────────
    app.add_handler(CallbackQueryHandler(start.cb_menu,          pattern="^menu_"))
    app.add_handler(CallbackQueryHandler(start.cb_menu,          pattern="^back_start$"))
    ...
    logger.info("🚀 Bot started! Anime Watchlist bot ready~")
